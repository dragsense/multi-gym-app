...
export type TTaskDeleteExtraProps = Record<string, unknown>;

type ITaskDeleteProps = THandlerComponentProps<
  TSingleHandlerStore<ITask, TTaskDeleteExtraProps>
>;
...
export default function TaskDelete({
  ...
  const selector = useShallow(
    (state: ISingleHandlerState<ITask, TTaskDeleteExtraProps>) => ({
      action: state.action,
      extra: state.extra,
  ...
  const taskDueDate = task?.dueDate
    ? formatDate(task.dueDate, settings)
    : "";
          ...
          <div className="rounded-lg border border-red-200 bg-red-50 p-4">
            <div className="text-sm">
              ...
              <div className="text-red-700">
                {taskDueDate && (
                  <div>
                    {buildSentence(t, "due", "date")}: {taskDueDate}
                  </div>
                )}
                {task.priority && (
                  <div>
                    {buildSentence(t, "priority")}: {task.priority}
                  </div>
                )}
                {task.status && (
                  <div>
                    {buildSentence(t, "status")}: {task.status.replace('_', ' ')}
                  </div>
                )}
              </div>
            </div>
