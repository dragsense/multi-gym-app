...
interface IBusinessBillingsTabProps {
  business: IBusiness;
  storeKey: string;
}

export function BusinessBillingsTab({ business, storeKey }: IBusinessBillingsTabProps) {
  const componentId = useId();
  const queryClient = useQueryClient();
  const { defaultPaymentMethod, isLoading: isLoadingDefaultPaymentMethod } = useDefaultCard(business.user?.id || "");
        ...
        <>
          ...
          >
            <ListHandler<IBilling, TBillingListData, IBillingListExtraProps, IBilling>
              queryFn={(params) => fetchUserBillings(business.user?.id || "", params)}
              initialParams={{
