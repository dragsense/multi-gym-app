...
import {
  useSearchableAccessHours,
  useSearchableAccessFeatures,
  useSearchableDoors,
...
import type {
  TMembershipData,
  TUpdateMembershipData,
} from "@shared/types/membership.type";
...
const AccessFeaturesSelect = React.memo((props: TCustomInputWrapper) => {
  ...
  return (
    <SearchableInputWrapper<IAccessFeature>
      ...
      getLabel={(item) => {
        if (!item?.name)
          return buildSentence(t, "select", "access", "features");
        return item.name;
      }}
      getKey={(item) => item.id.toString()}
      getValue={(item) => {
...
const TermsAndConditionsButton = React.memo((props: TCustomInputWrapper) => {
  ...
  const { watch, setValue } = useFormContext<
    TMembershipData | TUpdateMembershipData
  >();
...
interface IMembershipFormModalProps extends THandlerComponentProps<
  TFormHandlerStore<
    TMembershipData,
    TMembershipResponse,
    IMembershipFormModalExtraProps
  >
> {}
  ...
  const fields = useMemo(
    () =>
      ({
        ...
        cancellationFee: {
          ...(storeFields as TFieldConfigObject<TMembershipData>)
            .cancellationFee,
      ...
      }) as TFieldConfigObject<TMembershipData>,
    [storeFields, t],
  );
