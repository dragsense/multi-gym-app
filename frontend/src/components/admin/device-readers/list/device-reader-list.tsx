...
// Local
import { itemViews } from "./device-reader-item-views";
import { DeviceReaderFilters } from "./device-reader-filters";
...
export interface IDeviceReaderListExtraProps { }

interface IDeviceReaderListProps extends TListHandlerComponentProps<TListHandlerStore<IDeviceReader, any, IDeviceReaderListExtraProps>,
  TSingleHandlerStore<IDeviceReader, any>> {
}

type ViewType = "table" | "list";

export default function DeviceReaderList({
  storeKey,
  store,
  singleStore
}: IDeviceReaderListProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-1 justify-between items-start md:items-center gap-2 flex-wrap">
        <DeviceReaderFilters store={store} />
        <ViewToggle componentId={componentId} />
        <Button onClick={handleCreate} data-component-id={componentId}>
          <Plus />{" "}
          <span className="hidden sm:inline capitalize">
            {buildSentence(t, "add", "device", "reader")}
          </span>
        </Button>
      </div>

      <TabsContent value="table">
        <TTable<IDeviceReader>
          listStore={store}
          columns={columns}
          emptyMessage={buildSentence(t, "no", "deviceReaders", "found")}
          showPagination={true}
        />
      </TabsContent>
