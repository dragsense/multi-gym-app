...
// Local
import { itemViews } from "./trainer-service-item-views";
import { TrainerServiceFilters } from "./trainer-service-filters";
...
export interface ITrainerServiceListExtraProps { }

interface ITrainerServiceListProps extends TListHandlerComponentProps<TListHandlerStore<ITrainerService, any, ITrainerServiceListExtraProps>,
  TSingleHandlerStore<ITrainerService, any>> {
}

type ViewType = "table" | "list";

export default function TrainerServiceList({
  storeKey,
  store,
  singleStore
}: ITrainerServiceListProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-1 justify-between items-start md:items-center gap-2 flex-wrap">
        <TrainerServiceFilters store={store} />
        <ViewToggle componentId={componentId} />
        <Button onClick={handleCreate} data-component-id={componentId}>
          <Plus />{" "}
          <span className="hidden sm:inline capitalize">
            {buildSentence(t, "add", "trainer", "service")}
          </span>
        </Button>
      </div>

      <TabsContent value="table">
        <TTable<ITrainerService>
          listStore={store}
          columns={columns}
          emptyMessage={buildSentence(t, "no", "trainerServices", "found")}
          showPagination={true}
        />
      </TabsContent>
