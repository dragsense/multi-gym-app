...
// Local
import { itemViews } from "./facility-info-item-views";
import { FacilityInfoFilters } from "./facility-info-filters";
...
export interface IFacilityInfoListExtraProps { }

interface IFacilityInfoListProps extends TListHandlerComponentProps<TListHandlerStore<IFacilityInfo, any, IFacilityInfoListExtraProps>,
  TSingleHandlerStore<IFacilityInfo, any>> {
}

type ViewType = "table" | "list";

export default function FacilityInfoList({
  storeKey,
  store,
  singleStore
}: IFacilityInfoListProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-1 justify-between items-start md:items-center gap-2 flex-wrap">
        <FacilityInfoFilters store={store} />
        <ViewToggle componentId={componentId} />
        <Button onClick={handleCreate} data-component-id={componentId}>
          <Plus />{" "}
          <span className="hidden sm:inline capitalize">
            {buildSentence(t, "add", "facility", "info")}
          </span>
        </Button>
      </div>

      <TabsContent value="table">
        <TTable<IFacilityInfo>
          listStore={store}
          columns={columns}
          emptyMessage={buildSentence(t, "no", "facilityInfo", "found")}
          showPagination={true}
        />
      </TabsContent>
