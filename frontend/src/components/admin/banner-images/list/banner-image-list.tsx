...
// Local
import { itemViews } from "./banner-image-item-views";
import { BannerImageFilters } from "./banner-image-filters";
...
export interface IBannerImageListExtraProps { }

interface IBannerImageListProps extends TListHandlerComponentProps<TListHandlerStore<IBannerImage, any, IBannerImageListExtraProps>,
  TSingleHandlerStore<IBannerImage, any>> {
}
  ...
  return (
    <div className="p-2" data-component-id={componentId}>
      <div className="flex flex-1 justify-between items-start md:items-center gap-4 mb-6">
        <BannerImageFilters store={store} />
        <Button
      ...
      <TList<IBannerImage>
        listStore={store}
        emptyMessage={buildSentence(t, 'no', 'banner', 'images', 'found')}
        showPagination={true}
        renderItem={listItem} 
