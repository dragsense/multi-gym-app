...
import {
  ...
  DateRangePicker,
  DateTimeRangePicker,
} from "@/components/form-ui/date-picker";
        ...
        <FormItem>
          ...
          >
            {field.label && !hideLabel && (
              <FormLabel
                className={`text-muted-foreground ${field.lableOrder || EORDER_CLASSES.First}`}
              >
                {field.label}
          ...
          <FormMessage />
          {field.bottomAdornment && (
            <FormDescription>{field.bottomAdornment}</FormDescription>
          )}
        </FormItem>
      )}
    />
      ...
      {({ field: controllerField, isDisabled }) => {
        return (
          <Input
            ...
            type={field.type}
            onChange={(e) =>
              controllerField.onChange(onChangeHandler(e.target.value))
            }
    ...
    >
      {({ field: controllerField, isDisabled }) => (
        <DateRangePicker
          value={controllerField.value}
          onChange={(dateRange) =>
            controllerField.onChange(onChangeHandler(dateRange))
          }
          placeholder={field.placeholder}
          disabled={isDisabled}
      ...
      {({ field: controllerField, isDisabled }) => (
        <DateTimeRangePicker
          ...
          onChange={(dateTimeRange) =>
            controllerField.onChange(onChangeHandler(dateTimeRange))
          }
          placeholder={field.placeholder}
          disabled={isDisabled}
          className={field.className}
